...
class DocumentController extends Controller
{
    ...
    public function createDocument($request, $response)
    {
        ...
        $payload = array(
            ...
            'type' => $data->type ? htmlentities(stripcslashes(strip_tags($data->type))) : '',
    ...
    public function uploadDocument($request, $response)
    {
        ...
        if (!$files || !$files['file']) {
            $response->code(400)->json('Ошибка загрузки файла. Повторите попытку позже.');
        ...
        $dir = $_SERVER['DOCUMENT_ROOT'] . '/uploads/documents/';
        ...
        $response->code(200)->json($fileName);
    }
}