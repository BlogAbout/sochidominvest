...
import classes from './RentPage.module.scss'
...
const RentPage: React.FC = () => {
    ...
    const [layout, setLayout] = useState<'list' | 'till' | 'map'>(getLayout('rents') === 'list' ? 'till' : getLayout('rents'))
    ...
    useEffect(() => {
        if (isUpdate) {
            ...
            BuildingService.fetchBuildings({active: [1], publish: 1, rent: [1]})
                .then((response: any) => {
            ...
            <Placemark key={building.id}
                       ...
                       onClick={() => {
                           openPopupBuildingInfo(document.body, {
                               building: building,
                               onClick: () => navigate('/rent/' + building.id)
                           })
                       }}
            />
        )
    ...
    const renderBuildingItem = (building: IBuilding, index: number) => {
        ...
        return (
            <div key={building.id} className={classes.item} onClick={() => navigate('/rent/' + building.id)}>
                    ...
                    <div className={classes.itemInfo}>
                        ...
                        {building.rentData ?
                            <div className={classes.counter}>
                                {numberWithSpaces(round(building.rentData.cost || 0, 0))} руб.
                                {building.rentData.type === 'short' ? '/в сутки' : '/в месяц'}
                            </div>
                        : null
                        }
    ...
    return (
        <main className={classes.RentPage}>
            <PageInfo title='Аренда'/>
